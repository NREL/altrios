...
replicate_times = 5
daily_throughput_range = range(200, 2001, 20)
train_batch_size_range = range(20, 221, 20)
...
for daily_throughput in daily_throughput_range:
    for train_batch_size in train_batch_size_range:
        ...
        hostler_num = math.ceil((hostler_cycle_time + crane_loading_time) / (state.CONTAINERS_PER_CRANE_MOVE_MEAN)) - 1
        ...
        print("-" * 100)
        print(f"current throughput: {daily_throughput}, batch size: {train_batch_size}")
        subprocess.run(["python", "single_track_simulation.py"], check=True)

        with open("performance_matrix.json", "r") as f:
            performance_data = json.load(f)

        # single_run = [total_delay_time, average_container_delay_time, ic_avg_time, oc_avg_time, total_avg_time, ic_energy, oc_energy, total_avg_energy]
        ...
        current_summary = {
            ...
            "Total delay time": round(performance_data["total_delay_time"],4),
            "Processing time (avg)": {
                "IC": round(performance_data["ic_avg_time"],4) + round(performance_data["average_container_delay_time"], 4),
                "OC": round(performance_data["oc_avg_time"], 4) + round(performance_data["average_container_delay_time"], 4),
                "Total": round(performance_data["total_avg_time"], 4) + round(performance_data["average_container_delay_time"], 4),
            },
            "Energy consumption (avg)": {
                "IC": round(performance_data["ic_energy"], 4),
                "OC": round(performance_data["oc_energy"], 4),
                "Total": round(performance_data["total_energy"], 4)
            }
        }
        ...
        delay_time = performance_data["total_delay_time"]
        average_container_delay_time = performance_data["average_container_delay_time"]
        ic_avg_time = performance_data["ic_avg_time"]
        oc_avg_time = performance_data["oc_avg_time"]
        total_avg_time = performance_data["total_avg_time"]
        ...
        performance_matrix.append([
                ...
                M, N, n_t, n_p, n_r,
                cranes, hostlers, average_container_delay_time,
                ic_avg_time, oc_avg_time, total_avg_time,
