...
from altrios import defaults, metric_calculator, sim_manager
...
def simulate_prescribed_rollout(
    ...
    demand_file: pl.DataFrame | Path | str = defaults.DEMAND_FILE,
    train_planner_config: planner_config.TrainPlannerConfig = planner_config.TrainPlannerConfig(),
    count_unused_locomotives=False,
    write_complete_results: bool | None = False,
    write_metrics: bool | None = False,
    ...
    for idx, scenario_year in enumerate(years):
        ...
        if write_complete_results:
            sims.to_file(
                str(results_folder / f"RolloutResults_Year {scenario_year}_Demand {Path(demand_file).name}.json"
                    ,
                )
                .replace(".csv", ""))

            train_consist_plan.write_csv(
                str(results_folder / f"ConsistPlan_Year {scenario_year}_Demand {Path(demand_file).name}.csv"
                    ,
                )
                .replace(".csv", "") + ".csv")
    ...
    if write_metrics:
        print
        (results_folder /
            f"Metrics_Demand {scenario_year}_DemandFile {os.path.basename(demand_file)}.xlsx".replace(".csv", "")
        )
        metrics.to_pandas().to_excel(
            results_folder / f"Metrics_Demand {scenario_year}_DemandFile {os.path.basename(demand_file)}.xlsx".replace(".csv", ""),
        )
