    ...
    fn derived_mass(&self) -> anyhow::Result<Option<si::Mass>> {
        if let (Some(baseline), Some(ballast)) = (self.baseline_mass, self.ballast_mass) {
            match self.loco_type {
                ...
                LocoType::Dummy(_) => {
                    bail!("`baseline` and `ballast` mass must be `None` with Dummy locomotive.")
                }
            }
            ...
            bail!("Both `baseline` and `ballast` masses must be either `Some` or `None`")
        }
    }
