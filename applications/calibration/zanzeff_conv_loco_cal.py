"""Script for calibrating conventional locomotive"""
...
def get_conv_trip_mods(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df39xx["Fuel Energy [J]"] = (
        (df39xx["Fuel Power [W]"] * cval.get_delta_seconds(df39xx["timestamp"])).cumsum().copy()
    )
...
class ModelError(cval.ModelError):
    def update_params(self, xs: list[float]) -> dict[str, LocomotiveSimulation]:
        """
        ...
        for key in return_model_dict:
            for path, x in zip(self.params, xs):
                return_model_dict[key] = alt.set_param_from_path(return_model_dict[key], path, x)
...
def get_mod_err(
    ...
    mod_err = ModelError(
        bincode_model_dict={key: df_sim_tup[1] for key, df_sim_tup in df_and_sim_dict.items()},
...
if __name__ == "__main__":
    ...
    parser.add_argument("--debug", action="store_true", help="If passed, runs in debug mode.")
    parser.add_argument(
        "--seed",
        type=int,
        default=42,
    ...
    objectives = [("Fuel Energy [J]", "loco_unit.fc.history.energy_fuel_joules")]
    ...
    for i, file in enumerate(cal_mod_files):
        ...
        cal_mod_dfs[file.stem] = get_conv_trip_mods(cal_mod_raw_dfs[file.stem])
        cal_df_and_sims[file.stem] = (cal_mod_dfs[file.stem], get_loco_sim(cal_mod_dfs[file.stem]))
    ...
    for i, file in enumerate(val_mod_files):
        ...
        val_mod_dfs[file.stem] = get_conv_trip_mods(val_mod_raw_dfs[file.stem])
        val_df_and_sims[file.stem] = (val_mod_dfs[file.stem], get_loco_sim(val_mod_dfs[file.stem]))
