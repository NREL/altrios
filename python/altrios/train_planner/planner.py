...
from altrios.train_planner import data_prep, planner_config, schedulers, train_demand_generators
    ...
    if (arrived.sum() > 0):
        ...
        for charger_type in charger_type_breakouts:
            ...
            for i in range(0, refueling_done_times.len()):
                ...
                if next_done.len() == 0: next_done = arrival_times[i]  # No wait if ports available
                else: next_done = max(next_done[0], arrival_times[i])  # Either wait for port or arrival
...
def run_train_planner(
    ...
    train_consist_plan = pl.DataFrame(schema={"Train_ID": pl.Int64,
         "Train_Type": pl.Utf8,
