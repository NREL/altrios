...
impl TrainSimBuilder {
    ...
    // TODO: Add loco_con mass somewhere here
...
pub fn run_speed_limit_train_sims(
    ...
    let parallelize = parallelize.unwrap_or_default();
    if parallelize {
        speed_limit_train_sims
            .0
            .iter_mut()
            .zip(timed_paths.iter())
            .enumerate()
            .try_for_each(|(i, (slts, tp))| {
                slts.walk_timed_path(&network, tp)
                    .map_err(|err| err.context(format!("train sim idx: {}", i)))
            })?;
    } else {
        speed_limit_train_sims
            .0
            .par_iter_mut()
            .zip(timed_paths.par_iter())
            .enumerate()
            .try_for_each(|(i, (slts, tp))| {
                slts.walk_timed_path(&network, tp)
                    .map_err(|err| err.context(format!("train sim idx: {}", i)))
...
#[cfg(feature = "pyo3")]
#[pyfunction]
pub fn build_and_run_speed_limit_train_sims(
    train_sim_builders: Vec<TrainSimBuilder>,
    network: Vec<Link>,
    timed_paths: Vec<Vec<LinkIdxTime>>,
    rail_veh_map: RailVehicleMap,
...
mod tests {
    ...
    fn test_make_train_params() {
        ...
        let rail_vehicle_map =
            import_rail_vehicles(Path::new("./src/train/rail_vehicles.csv")).unwrap();
    ...
    fn test_make_speed_limit_train_sims() {
        ...
        rvm_file.push("altrios-core/src/train/rail_vehicles.csv");
        let rail_vehicle_map = match import_rail_vehicles(&rvm_file) {
            Ok(rvm) => rvm,
            Err(_) => import_rail_vehicles(Path::new("./src/train/rail_vehicles.csv")).unwrap(),
        };
