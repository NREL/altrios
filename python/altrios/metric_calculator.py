...
def import_emissions_factors_cambium(
    ...
    location_ids = [
        loc.location_id for loc_list in location_map.values() for loc in loc_list
    ]
    grid_emissions_regions = [
        loc.grid_emissions_region
...
def import_energy_prices_eia(
    ...
    electricity_price_regions = [
        loc.electricity_price_region
        for loc_list in location_map.values()
        for loc in loc_list
    ]
    liquid_fuel_price_regions = [
        loc.liquid_fuel_price_region
        for loc_list in location_map.values()
        for loc in loc_list
    ]
...
def add_battery_costs(loco_info: pd.DataFrame, year: int) -> pd.DataFrame:
    ...
    for idx, row in loco_info.iterrows():
        if (hasattr(row["Rust_Loco"], "res")) and row["Rust_Loco"].res is not None:
            # Chad & Matt...  I don't like this line because it ties itself to BELs.  This will not work long term.
            kW = (
                row["Rust_Loco"].to_pydict()["loco_type"]["BatteryElectricLoco"]["res"][
                    "pwr_out_max_watts"
                ]
                / 1000
            )
