...
def append_loco_info(loco_info: pd.DataFrame) -> pd.DataFrame:
    ...
    get_mass_ton = lambda loco: (
        0 if not loco.mass_kg else loco.mass_kg / alt.utils.KG_PER_TON
    )
    get_starting_soc = lambda loco: (
        defaults.DIESEL_TANK_CAPACITY_J
        if not loco.res
        else loco.res.to_pydict()["state"]["soc"]
        * loco.res.to_pydict()["energy_capacity_joules"]
    )
    get_min_soc = lambda loco: (
        0
        if not loco.res
        else loco.res.to_pydict()["min_soc"]
        * loco.res.to_pydict()["energy_capacity_joules"]
    )
    get_max_soc = lambda loco: (
        defaults.DIESEL_TANK_CAPACITY_J
        if not loco.res
        else loco.res.to_pydict()["max_soc"]
        * loco.res.to_pydict()["energy_capacity_joules"]
    )
    get_capacity = lambda loco: (
        defaults.DIESEL_TANK_CAPACITY_J
        if not loco.res
        else loco.res.to_pydict()["energy_capacity_joules"]
    )
...
def appendTonsAndHP(
    ...
    def get_kg_empty(veh):
        return (
            veh.to_pydict()["mass_static_base_kilograms"]
            + veh.to_pydict()["axle_count"]
            * veh.to_pydict()["mass_rot_per_axle_kilograms"]
        )

    def get_kg(veh):
        return (
            veh.to_pydict()["mass_static_base_kilograms"]
            + veh.to_pydict()["mass_freight_kilograms"]
            + veh.to_pydict()["axle_count"]
            * veh.to_pydict()["mass_rot_per_axle_kilograms"]
        )
