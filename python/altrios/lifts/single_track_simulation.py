...
layout.load_layout_config_from_json()
...
def truck_entry(env, train_schedule, terminal, truck_id):
    ...
    with terminal.in_gates.request() as gate_request:
        ...
        record_vehicle_event('truck', truck_id, f'drop off {oc_id}', 'loaded', 'load', truck_unload_time, emissions, 'end',
                             env.now)
        ...
        record_vehicle_event('truck', truck_id, f'trip {oc_id}', 'empty', 'trip', truck_travel_time, emissions, 'end',
                             env.now)
...
def container_process(env, terminal, train_schedule, all_oc_prepared, oc_needed, all_ic_unload_event, all_ic_picked):
    ...
    # Assign a hostler to pick up an OC, if balanced loop remains
    if len(terminal.parking_slots.items) > 0:
        ...
        hostler_reposition_travel_time = simulate_reposition_travel(hostler_id, current_veh_num, total_lane_length,
                                                                    d_h_min, d_h_max)
...
def process_train_arrival(env, terminal, train_departed_event, train_schedule, next_departed_event):
    ...
    print(
        f"------------------- The current train is {train_id}: scheduled arrival time {arrival_time}, OC {oc_needed}, IC {total_ic} -------------------")
    ...
    if env.now <= arrival_time:
        ...
        print(
            f"Time {env.now}: [In Time] Train {train_schedule['train_id']} has arrived, waiting to be assigned to the track {track_id}.")
        delay_time = 0
    else:
        ...
        print(
            f"Time {env.now}: [DELAYED] Train {train_schedule['train_id']} has been delayed for {delay_time} hours, waiting to be assigned to the track {track_id}.")
    ...
    for ic_id in range(state.IC_NUM, state.IC_NUM + train_schedule['full_cars']):
        record_event(ic_id, 'train_arrival',
    ...
    for oc_id in range(state.OC_NUM, state.OC_NUM + train_schedule['oc_number']):
        record_event(f"OC-{oc_id}", 'train_depart',
...
def run_simulation(train_consist_plan: pl.DataFrame, terminal: str, out_path=None):
    ...
    train_timetable = [
        {'train_id': 840, 'arrival_time': 0, 'departure_time': 6, 'empty_cars': 0, 'full_cars': 150, 'oc_number': 150,
         'truck_number': 150},
        {'train_id': 7, 'arrival_time': 6, 'departure_time': 12, 'empty_cars': 0, 'full_cars': 150, 'oc_number': 150,
         'truck_number': 150},
        {'train_id': 82, 'arrival_time': 12, 'departure_time': 18, 'empty_cars': 0, 'full_cars': 150, 'oc_number': 150,
         'truck_number': 150},
        {'train_id': 414, 'arrival_time': 18, 'departure_time': 18, 'empty_cars': 0, 'full_cars': 50, 'oc_number': 50,
         'truck_number': 50}]
